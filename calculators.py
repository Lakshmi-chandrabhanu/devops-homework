...
def calculate_heart_score(history, ecg, age_score, risk_factors, troponin):
    ...
    for param in params:
        if param not in [0, 1, 2]:
            raise ValueError("Invalid parameter value. Each parameter must be 0, 1, or 2.")
...
def calculate_pecarn(age_months, gcs, altered_mental_status,
    ...
    **Age ≥ 24 months**
    ...
    if risk_level == 'high':
        ...
        interpretation = 'CT ' \
        'can versus observation: individualise based on physician experience, multiple vs isolated findings, worsening symptoms, age < 3 months, parental preference'
    else:
